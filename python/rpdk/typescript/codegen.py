...
EXECUTABLE = "cfn"
SUPPORT_LIB_NAME = "@extend/cfn-resource-cli"
...
def _load_support_lib_version() -> str:
    ...
    (e.g. ``2.0.0``).  This function returns the exact version suitable
    for ``package.json`` dependency declarations.
    ...
    try:
        ...
        LOG.warning(
            "Could not read support-lib-version.txt;"
            " falling back to '2.0.0'."
        )
        return "2.0.0"
...
class TypescriptLanguagePlugin(LanguagePlugin):
    ...
    def init(self, project):
        ...
        def _render_template(path, template_name=None, **kwargs):
            LOG.debug("Writing '%s'", path)
            template = self.env.get_template(template_name or path.name)
        ...
        _render_template(
            ...
            template_name="package.json.tpl",
